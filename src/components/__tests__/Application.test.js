...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    //5. expects the spots remaining to have not changed
    const day = getAllByTestId(container, "day").find(day =>
      queryByText(day, "Monday")
    );
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    fireEvent.click(getByText(appointment, "Save"));
    //4. expects the save error to show
    await waitForElement(() => (getByText(container, "Could not save your appointment")))
  ...
  it("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    await waitForElement(() => getByText(container, "Archie Cohen"));
    //1.5 find the appointment
    const appointment = getAllByTestId(container, "appointment").find(
      appointment => queryByText(appointment, "Archie Cohen"));
    //2. clicks the delete appointment button
    fireEvent.click(getByAltText(appointment, "Delete"));
    fireEvent.click(getByText(appointment, "Confirm"));

    //3. expects error
    await waitForElement(() => (getByText(container, "Could not delete your appointment")));
  });
});
